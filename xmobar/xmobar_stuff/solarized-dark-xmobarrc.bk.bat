...
--                    , Run Com ".local/bin/pacupdate" [] "pacupdate" 36000
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = "%UnsafeStdinReader% } <fc=#da8548>%%   <fc=#da8458><action=`alacritty -e htop`>%cpu%</action></fc>   %temp%</fc><fc=#da8548>%%  %fan%</fc>  <fc=#da8548>%baticon%  %battery%</fc>{ <fc=#46d9ff><action=`emacsclient -c -a 'emacs' --eval '(doom/window-maximize-buffer(dt/year-calendar))'`>%date%</action></fc> %trayerpad%"
       }
